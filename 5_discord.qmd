...
私は[@bot3]を参考に，メッセージに対して国旗のスタンプをつけるとその言語に翻訳してDMで送ってくれる仕様に変更した．[GitHub](https://github.com/yoshiyuki-miyamori/MacBookEnv)にdeepl.pyとして置いている．
...
　macOSのパソコンをサーバーとして使う場合に，Botの再起動を(半)自動化するのに私が試した方法3つを紹介する．共存させるのはあまり良くないので，別の方法に変える時は前の設定を消すこと．最初のターミナルと同期させるのは自然に思いつく方法であり，少し美しさに欠けるものの非常に簡単に実装できる．
残り2つはlaunchdというmacOSの機能を使うもので，ログイン時に起動させるLaunchAgentsと電源を入れた時に起動させるLaunchDaemonsがある．LaunchDaemonsはログイン前から実行させるので，必然的にパソコンの奥に入る必要があり設定が面倒である．LaunchAgentsは機能も良く，設定も基本的にホームディレクトリの下でできるので比較的簡単であり，バランスが良い．とはいえ機能はLaunchDaemonsが一番良いかと思いきや，実はインターネット接続等の問題で結局ログインしないとBotが起動しなかったので，
結局現在はLaunchAgentsに落ち着いている．
...
<dict>
    ...
    <key>UserName</key>
    <string>room261</string>

    <key>ProgramArguments</key>
    <array>
        <string>/Users/room261/Documents/Discord/.venv/bin/python3</string>
        <string>/Users/room261/Documents/Discord/deepl.py</string>
    </array>
    
    <key>WorkingDirectory</key>
    <string>/Users/room261/Documents/Discord</string>

    <key>StandardOutPath</key>
    <string>/Users/room261/Documents/Discord/.deepl.log</string>
...
　ログインする前から動かすのだから当然ユーザーのDocumentsにあるファイルを読み込むことなどできない．
つまり，先ほど作成したプロジェクトのフォルダを/usr/local/に作り直す必要がある．
...
```
ここでmkdirはmake directoryである．
今このフォルダの所有者はrootになっているのでユーザーに変更する：
```zsh
sudo chown room261 /usr/local/Discord
```
ここでchownはchange ownerであり，/usr/local/Discordの所有者をroom261に変更している．
...
<dict>
    ...
    <array>
        ...
        <string>/usr/local/Discord/deepl.py</string>
    </array>
    ...
    <key>StandardOutPath</key>
    <string>/usr/local/Discord/.deepl.log</string>
...
```
::: callout-tip
実は直前の二文は次の一文にまとめることもできる．
```zsh
sudo launchctl kickstart -k system/deepl
```
ctlはcontrolである．systemドメインのdeeplというラベルがついたものを起動している．
(kickstartというのはstartの現代改良版コマンドである．)
-kはkillオプションであり，既に実行中の場合はそれを停止してくれる．
:::
