...
cd ~/Desktop
```
と流してみよう．`ユーザー名@ホスト名 Desktop % `に変わったはずである．ホームディレクトリは`~`で表したが，カレントディレクトリは`.`で表せる．これは案外便利である．また，現在より一つ上の親ディレクトリは`..`で表せる．試しに
```zsh
cd ..
```
と流してみよう．ホームディレクトリに戻ってきたはずである．

::: callout-tip
　実は`cd`だけでもホームディレクトリに戻れる．また，`cd カレントディレクトリ/目的のディレクトリ`の形の時は，もちろん`cd ./目的のディレクトリ`と書いてもいいし，実は`cd 目的のディレクトリ`まで省略しても良い．だからさっきの`cd ~/Desktop`は`cd ./Desktop`でも`cd Desktop`でも良かった．
...
```
titleはなんでも良いが，例えばMy MacBook SSH Keyなど相手のローカルを特定する名前が良いだろう．keytypeはそのまま．
